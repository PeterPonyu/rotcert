#!/usr/bin/env python3
"""R=20 scene-split repeats for a GWD G1 certificate (generic, dataset-agnostic).
Usage: r20_generic.py <matched.jsonl> <out.json>
Same protocol as the ORCNN/DIOR run: 20 repeated scene-level splits (40/20/40, split-seed
...
(verified 2026-07-13). Everything else -- including the historical "experiment" label --
is kept verbatim so the byte-repro guarantee holds.
"""
import json, os, sys, time
import numpy as np
from scipy.stats import bootstrap as scipy_bootstrap

ROOT = os.environ.get("ROTCERT_ROOT") or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
